...
int main(void) {
    ...
    if (get_core_id() == 0) {
        ...
        unsigned int dst_strides[] = {5, 12, 24, 64};   // Number of strides in destination memory
...
int test_idma_2D(uint32_t size, test_type_t type, uint32_t ext_addr, uint32_t tcdm_addr,
    ...
    if (type == L2_TO_L1) {
        ...
        id = pulp_cl_idma_L2ToL1_3d((unsigned int)src_ptr, (unsigned int)dst_ptr, length, src_stride, dst_stride, num_reps);
    } else if (type == L1_TO_L2) {
        src_ptr = (uint8_t *)tcdm_addr;
        dst_ptr = (uint8_t *)ext_addr;
        for (uint32_t i = 0; i < size; i++) src_ptr[i] = (uint8_t)(i & 0xFF);
        id = pulp_cl_idma_L1ToL2_3d((unsigned int)src_ptr, (unsigned int)dst_ptr, length, src_stride, dst_stride, num_reps);
    }

#ifdef VERBOSE
    printf("DMA status (before barrier): %d\n", plp_cl_dma_status(id));
#endif
    plp_dma_barrier(id);
    // plp_dma_wait(id);
#ifdef VERBOSE
    printf("DMA status (after barrier): %d\n", plp_cl_dma_status(id));
    print_memory((uint8_t *)ext_addr, size, "ext");
    print_memory((uint8_t *)tcdm_addr, size, "loc");
#endif
