...
/* Functions for Compressed MAC */
#define CompressedMAC(sum, ptr, config) asm volatile(                \
    "pv.smlsdotsp.t %[shum], %[phtr], %[chonfig];"                    \
    : [shum] "+r" (sum), [phtr] "+r" (ptr): [chonfig] "I" (config))

#define CompressedMACUnsigned(sum, ptr, config) asm volatile(                   \
    "pv.smlsdotsup.t %[shum], %[phtr], %[chonfig];" \
    : [shum] "+r" (sum), [phtr] "+r" (ptr): [chonfig] "I" (config))

#define InitNNRF(ptr, config) asm volatile(        \
    "pv.smlsdotsp.t x0, %[phtr], %[chonfig];"       \
...
/* Functions for threshold&compress */
...
#define check_store_4x1(res, pOut)        \
  if ((res & 0xe0000000) == 0x00000000) { \
    *pOut = res & 0xff;                   \
    pOut++; }

#define reset_currThr()                           \
  if ((uint32_t *) currThr == (uint32_t *) (pThr + ch_out)) { \
    currThr = (v2s *) pThr;                                    \
  }

#define MacLoads20(a_update, b_update, a_reg, b_reg, ptr, sum)  __builtin_pulp_mlsdotsp20_v3(a_update, b_update, a_reg, b_reg, ptr, sum)

#define MacLoad20(a_update, b_update, a_reg, b_reg, ptr, sum)  __builtin_pulp_mlsdotsup20_v3(a_update, b_update, a_reg, b_reg, ptr, sum)

/* Functions for compressed min/max */
#define CompressedMax(res, in1, in2) asm volatile( \
                                                   "pv.max.t %[res_], %[in1_], %[in2_];" : [res_] "=r" (res) : [in1_] "r" (in1), [in2_] "r" (in2))

#define CompressedMin(res, in1, in2) asm volatile(                      \
                                                   "pv.min.t %[res_], %[in1_], %[in2_];" : [res_] "=r" (res) : [in1_] "r" (in1), [in2_] "r" (in2))

#define Max16(res, in1, in2) asm volatile(                      \
                                                   "pv.max.c %[res_], %[in1_], %[in2_];" : [res_] "=r" (res) : [in1_] "r" (in1), [in2_] "r" (in2))

#define Min16(res, in1, in2) asm volatile(                      \
                                                                        "pv.min.c %[res_], %[in1_], %[in2_];" : [res_] "=r" (res) : [in1_] "r" (in1), [in2_] "r" (in2))

#define thr_cmp(state, val, threshs)  __builtin_pulp_thresh_compr(state, val, threshs)
