...

{
  ...
  // number of bytes:  5 els per byte
  int ch_im_in_r = ch_im_in/5;
  ...
  // logic is the same as for i2, except we use the ternary version of xpulp_nn_compare_and_replace...
  for (i_y = start; i_y < stop; i_y++)
  {
    for (i_x = 0; i_x < dim_im_out_x; i_x++)
    {
      ...
      {
        xpulp_tnn_compare_and_replace_if_larger_ternary(target, win_start, ch_im_in_r);
      }
    }
  }
#ifndef FC_TEST
  ...
  for (i_y = start2; i_y < stop2; i_y++)
  {
    ...
    {
      xpulp_tnn_compare_and_replace_if_larger_ternary(target, row_start, dim_im_out_x * ch_im_in_r);
    }
  }
  #ifndef FC_TEST
  pi_cl_team_barrier(0);
