...
int main(int argc, char *argv[])
{
  ...
  if (get_core_id() == 0) {
  printf("===> TEST 0: Running xpulp_nn_conv_u4_u8_i2...\n");
  printf("  dims_in     = [4, 4]\n");
  ...
  if (get_core_id() == 0) {
  printf("===> TEST 0: Finished running xpulp_nn_conv_u4_u8_i2\n");
  printf("num_cycles = %d\n", num_cycles);
  printf("MACs = 73728\n");
...
void test_0(void) {
  ...
  dma_tx.ext = pLambda_0;
  dma_tx.loc = lambda_l1;
  dma_tx.length_1d_copy = 32 * 4; // 4 bytes per lambda item
  dory_dma_memcpy_async(&dma_tx);
  dma_tx.ext = pKappa_0;
  dma_tx.loc = kappa_l1;
  dory_dma_memcpy_async(&dma_tx);
