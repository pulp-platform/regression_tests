...
void check_matrix_mul(testresult_t *result, void (*start)(), void (*stop)()) {
  ...
  chunk = SIZE / (num_cores/3); // We want to use one third the cores, so the chunk size must be ttiplicated.
  // lower bound
  // We define the same lower bound for each triplet of cores
  if (core_id < num_cores/3)
    lb = core_id * chunk;
  else if (core_id < 2*(num_cores/3))
    lb = (core_id - (num_cores/3)) * chunk;
  else
    lb = (core_id - 2*(num_cores/3)) * chunk;
  // upper bound
  ...
  for(i = lb; i < ub; i++) {
    for(j = 0; j < SIZE; j++) {
      if (core_id < num_cores/3) { // Group 1 checking on group 2 and group 3
        err[core_id] += (temp_res[core_id][i][j] != temp_res[core_id+(num_cores/3)][i][j]) &&
                        (temp_res[core_id][i][j] != temp_res[core_id+2*(num_cores/3)][i][j]);
      } else if (core_id < 2*(num_cores/3)) { // Group 2 checking on group 1 and group 3
        err[core_id] += (temp_res[core_id][i][j] != temp_res[core_id-(num_cores/3)][i][j]) &&
                        (temp_res[core_id][i][j] != temp_res[core_id+(num_cores/3)][i][j]);
      } else { // Gour 3 checking on grou 1 and group 2
        err[core_id] += (temp_res[core_id][i][j] != temp_res[core_id-2*(num_cores/3)][i][j]) &&
                        (temp_res[core_id][i][j] != temp_res[core_id-(num_cores/3)][i][j]);
      }
    }
  }
  ...
  if (core_id < num_cores/3){
    for(i = lb; i < ub; i++) {
      for(j = 0; j < SIZE; j++) {
        g_mC[i][j] = ((err[core_id] == 0) && (err[core_id+(num_cores/3)] == 0) && (err[core_id+2*(num_cores/3)] == 0)) ? temp_res[core_id][i][j] : -1;
      }
    }
  }
