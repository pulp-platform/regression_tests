...
#define MCHAN_COMMAND_QUEUE 0x10204400  // 0x10201800
#define MCHAN_STATUS_REGISTER 0x10204404  // 0x10201804
...
static inline int mchan_alloc() { return *(volatile int*)MCHAN_COMMAND_QUEUE; }

static inline void mchan_transfer(
    unsigned int len, char type, char incr, char twd_ext, char twd_tcdm,
    char ele, char ile, char ble, unsigned int ext_addr, unsigned int tcdm_addr,
    unsigned int ext_count, unsigned int ext_stride, unsigned int tcdm_count,
    unsigned int tcdm_stride) {
  *(volatile int*)MCHAN_COMMAND_QUEUE =
      len | (type << PLP_DMA_TYPE_BIT) | (incr << PLP_DMA_INCR_BIT) |
      (twd_ext << PLP_DMA_2D_BIT) | (ele << PLP_DMA_ELE_BIT) |
      (ile << PLP_DMA_ILE_BIT) | (ble << PLP_DMA_BLE_BIT) |
      (twd_tcdm << PLP_DMA_2D_TCDM_BIT);
...
static inline void fc_mchan_transfer(
    unsigned int len, char type, char incr, char twd_ext, char twd_tcdm,
    char ele, char ile, char ble, unsigned int ext_addr, unsigned int tcdm_addr,
    unsigned short int ext_count, unsigned short int ext_stride,
    unsigned short int tcdm_count, unsigned short int tcdm_stride) {
  *(volatile int*)PE_MCHAN_COMMAND_QUEUE =
      len | (type << PLP_DMA_TYPE_BIT) | (incr << PLP_DMA_INCR_BIT) |
      (twd_ext << PLP_DMA_2D_BIT) | (ele << PLP_DMA_ELE_BIT) |
      (ile << PLP_DMA_ILE_BIT) | (ble << PLP_DMA_BLE_BIT) |
      (twd_tcdm << PLP_DMA_2D_TCDM_BIT);
...
static inline void fc_mchan_barrier(int id) {
  while (((*(volatile int*)(PE_MCHAN_STATUS_REGISTER)) >> id) & 0x1) {
    if (hal_is_fc()) {
      ...
      eu_evt_maskWaitAndClr(1 << CL_DMA_EVENT);
  };
}
