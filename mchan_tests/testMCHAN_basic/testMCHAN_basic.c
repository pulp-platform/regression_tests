...
int testMCHAN(unsigned int len, char type, char incr, char twd_ext,
              char twd_tcdm, char ele, char ile, char ble,
              unsigned int ext_addr, unsigned int tcdm_addr,
...
int main() {
  ...
  /*  if (rt_core_id() == 0) */
  /*   { */
  /*     if ((ext = rt_alloc(RT_ALLOC_L2_CL_DATA, MAX_BUFFER_SIZE)) == 0) return
   * -1; */
  /*     if ((loc = rt_alloc(RT_ALLOC_CL_DATA, MAX_BUFFER_SIZE)) == 0) return
   * -1; */
  /*   } */
  ...
  if (get_core_id() == 0) {
    ...
    for (int i = 1; i < 16384; i = 2 * i) {
      error_count += testMCHAN(i, TX, INC, LIN, LIN, 1, 0, 0,
                               ((unsigned int)ext), TCDM_DATA_ADDR, 0, 0, 0, 0);
    }
    for (int i = 1; i < 16384; i = 2 * i) {
      error_count += testMCHAN(i, RX, INC, LIN, LIN, 1, 0, 0, EXT_DATA_ADDR,
                               TCDM_DATA_ADDR, 0, 0, 0, 0);
    }
    ...
    for (int i = 1; i < 16384; i = 2 * i) {
      error_count += testMCHAN(i, TX, INC, LIN, LIN, 1, 0, 0, EXT_DATA_ADDR,
                               TCDM_DATA_ADDR, 0, 0, 0, 0);
    }
    for (int i = 1; i < 16384; i = 2 * i) {
      error_count += testMCHAN(i, RX, INC, LIN, LIN, 1, 0, 0, EXT_DATA_ADDR,
...
int testMCHAN(unsigned int len, char type, char incr, char twd_ext,
  ...
  mchan_transfer(len, type, incr, twd_ext, twd_tcdm, ele, ile, ble, ext_addr,
                 tcdm_addr, ext_count, ext_stride, tcdm_count, tcdm_stride);
